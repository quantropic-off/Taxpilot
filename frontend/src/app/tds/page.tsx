...
import { FileText, Calculator, FileCheck2, AlertCircle, Upload, Plus, ArrowRight } from "lucide-react";
...
export default function TDSPractice() {
  ...
  const handleAddDeduction = async () => {
    if (!newDed.deductee_pan) {
      toast.error(extractError("Please enter a Deductee PAN."));
      return;
    }
  ...
  const handleImportExcel = () => {
    const mockDeductions = [
      { deductee_pan: "ABCDE1234F", section_code: "194J", payment_amount: 50000, deduction_date: "2024-06-10", tax_deducted: 5000 },
      { deductee_pan: "FGHIJ5678K", section_code: "194C", payment_amount: 100000, deduction_date: "2024-06-12", tax_deducted: 2000 }
    ];
    setDeductions(prev => [...prev, ...mockDeductions]);
    setErrors([]);
  };
      ...
      <div className="flex flex-col gap-5 h-[calc(100vh-120px)]">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button onClick={handleImportExcel} className="text-[13px] font-medium bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-50 transition-colors flex items-center gap-2">
              <Upload className="h-4 w-4" /> Import Excel
            </button>
            <button 
              onClick={() => {
                setIsPushing(true);
                localStorage.setItem('mock_tds_deductions', JSON.stringify(deductions));
                if (caseId) localStorage.setItem('mock_tds_caseId', caseId.toString());
                setTimeout(() => router.push("/mock/traces"), 1500);
              }}
              disabled={isPushing || deductions.length === 0 || errors.length > 0}
              className={`text-[13px] font-medium px-4 py-2 rounded-md flex items-center gap-2 transition-colors ${deductions.length > 0 && errors.length === 0 ? "bg-teal-600 text-white hover:bg-teal-700" : "bg-gray-100 text-gray-400 cursor-not-allowed pointer-events-none"}`}
            >
              {isPushing ? (
                <><span className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Syncing to TRACES...</>
              ) : (
                ...
                <table className="w-full text-left whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-100 text-[13px]">
                    ...
                    <tr className="bg-teal-50/30">
                      ...
                      <td className="px-2 py-2">
                        <select className="w-full text-[13px] p-1.5 pr-8 rounded border border-gray-300 focus:ring-1 focus:ring-teal-500 outline-none truncate" value={newDed.section_code} onChange={e => setNewDed({...newDed, section_code: e.target.value})}>
              ...
              <div className="flex-1 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col min-h-[400px]">
                ...
                <div className="p-4 flex-1">
                  <table className="w-full text-left whitespace-nowrap border border-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-100 text-[13px]">
                      <tr>
                        <td colSpan={4} className="px-4 py-12 text-center text-gray-400">No salary details added.</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div className="flex-1 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col min-h-[400px]">
                ...
                <div className="p-4 flex-1">
                  <table className="w-full text-left whitespace-nowrap border border-gray-200">
                    ...
                    <tbody className="divide-y divide-gray-100 text-[13px]">
                      <tr>
                        <td colSpan={5} className="px-4 py-12 text-center text-gray-400">No challans found.</td>
                      </tr>
                    </tbody>
                  </table>
                  {/* Removed Warning */}
