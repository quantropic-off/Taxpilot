...
export default function MockGSTPortal() {
  ...
  const handleCreateCase = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch("http://localhost:7860/api/v1/gst/cases", {
        method: "POST",
  ...
  const handleAddInvoice = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:7860/api/v1/gst/cases/${caseId}/invoices`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleGenerateGSTR1 = async () => {
    if (!caseId) return;
    const res = await fetch(`http://localhost:7860/api/v1/gst/cases/${caseId}/generate-gstr1`, { method: "POST" });
    const data = await res.json();
  ...
  const handleSimulateARN = async () => {
    if (!caseId) return;
    const res = await fetch(`http://localhost:7860/api/v1/gst/cases/${caseId}/simulate-arn`, { method: "POST" });
    const data = await res.json();
