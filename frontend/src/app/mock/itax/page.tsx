...
export default function MockITaxPortal() {
  ...
  const handleCreateCase = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch("http://localhost:8000/api/v1/itr/cases", {
        method: "POST",
  ...
  const handleComputeTax = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/v1/itr/cases/${caseId}/compute`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleFileITR = async () => {
    if (!caseId) return;
    const res = await fetch(`http://localhost:8000/api/v1/itr/cases/${caseId}/file`, { method: "POST" });
    const data = await res.json();
